...
module.exports.index = async (req, res) => {
  ...
  if (length > 0) {
    ...
    let shopIds = books.reduce((acc, curr) => {
      if (curr.shopId) acc.add(curr.shopId);
      return acc;
    }, new Set());
    shopIds = Array.from(shopIds.values());
    const shops = await User.find({ _id: { $in: shopIds } }, "myShop");
    books = books.map(book => {
      const shop = shops.find(s => s._id.toString() === book.shopId);
      if (shop) {
        book.shop = shop;
      }
      return book;
