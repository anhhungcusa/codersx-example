...
module.exports.addToCart = (req, res) => {
  ...
  const cartId = sessionId || shortId();
  if (!sessionId) {
    res.cookie("sessionId", cartId, { maxAge: 24 * 3600 * 1000, signed: true });
    db.get("sessions")
      .push({ id: cartId, books: [bookId] })
      .write();
    return res.redirect("/books");
  }
  const cartQuery = db.get("sessions").find({ id: cartId });
  console.log(cartQuery.value(), cartId, sessionId);
  let { books } = cartQuery.value();
  if (!books.includes(bookId)) books.push(bookId);
  cartQuery.assign({ books }).write();
  res.redirect("/books");
};
